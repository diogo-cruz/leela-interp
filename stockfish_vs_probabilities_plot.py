...
Uses the full dataset from study_puzzles_head_all.csv
"""
...
def evaluate_branches(puzzle_row: pd.Series, engine: chess.engine.SimpleEngine) -> dict:
    ...
    try:
        ...
        baseline_info = engine.analyse(board, chess.engine.Limit(time=0.5, depth=20))
        ...
        if branch_1 and branch_1_probs:
            try:
                ...
                info = engine.analyse(board_after, chess.engine.Limit(time=0.5, depth=20))
                score = info['score']
...
def main():
    print("="*70)
    print("STOCKFISH CENTIPAWN vs BRANCH PROBABILITY ANALYSIS")
    print("="*70)
    
    # Load data
    df = pd.read_csv('study_puzzles_head_all.csv')
    ...
    # Save data
    plot_df.to_csv('stockfish_vs_probabilities_data.csv', index=False)
    ...
    ax.set_title('Stockfish Evaluation Change vs Branch Probability', fontsize=14, fontweight='bold')
    ...
    bin_stats = plot_df.groupby('cp_bin')['prob'].agg(['mean', 'std', 'count'])
    ...
    plt.tight_layout()
    plt.savefig('stockfish_vs_probabilities_plot.png', dpi=300, bbox_inches='tight')
    print("\nPlot saved to stockfish_vs_probabilities_plot.png")
