...
on your system to add captions to chess boards.
"""
...
class IcebergBoardExtended(ice.DrawableWithChild):
    ...
    def setup(self):
        ...
        if self.next_moves is not None:
            ...
            for i, move in enumerate(next_moves):
                ...
                label = f"{i + 1}{'st' if i == 0 else 'nd' if i == 1 else 'rd' if i == 2 else 'th'}"
                arrow_labels.append((from_square, to_square, label))

        if self.attn_map is not None:
            for i, value in enumerate(self.attn_map):
        ...
        if self.arrows is not None:
            for i, (move, color) in enumerate(self.arrows.items()):
                from_square = CHESS_NAME_TO_SQUARE[move[:2]]
                to_square = CHESS_NAME_TO_SQUARE[move[2:4]]
        ...
        svg = self.add_arrow_labels(svg, arrow_labels)
    ...
    def add_arrow_labels(self, svg_string, arrow_labels):
        ...
        # Create labels
        for from_square, to_square, label in arrow_labels:
            mid_x = (chess.square_file(from_square) + chess.square_file(to_square)) / 2
            mid_y = (chess.square_rank(from_square) + chess.square_rank(to_square)) / 2
            text_elem = ET.Element('text', {
                'x': str(mid_x * 45 + 22.5),
                'y': str((7 - mid_y) * 45 + 22.5),
                'font-size': '12',
                'text-anchor': 'middle',
                'fill': 'black'
            })
    ...
    def setup(self):
        ...
        if self.next_moves is not None:
            ...
            for i, move in enumerate(next_moves):
                from_square = CHESS_NAME_TO_SQUARE[move[:2]]
                to_square = CHESS_NAME_TO_SQUARE[move[2:4]]
                arrow = chess.svg.Arrow(from_square, to_square, color="blue")
                arrow.label = f"{i + 1}st" if i == 0 else f"{i + 1}nd" if i == 1 else f"{i + 1}rd" if i == 2 else f"{i + 1}th"
                arrows.append(arrow)
        ...
        if self.arrows is not None:
            for i, (move, color) in enumerate(self.arrows.items()):
                from_square = CHESS_NAME_TO_SQUARE[move[:2]]
                to_square = CHESS_NAME_TO_SQUARE[move[2:4]]
                arrow = chess.svg.Arrow(from_square, to_square, color=color)
                arrow.label = f"{i + 1}st" if i == 0 else f"{i + 1}nd" if i == 1 else f"{i + 1}rd" if i == 2 else f"{i + 1}th"
                arrows.append(arrow)
        ...
        self.svg_string = svg  # Store the SVG string
        child = ice.SVG(raw_svg=svg)
