...
def main(args):
    ...
    puzzles = puzzles[puzzles.principal_variation.apply(lambda x:len(x) == 9)]
    print("Number of puzzles after filtering for PV length 9:", len(puzzles)) #63198
    ...
    puzzles = puzzles[puzzles.principal_variation.apply(lambda x: x[0][2:4] != x[8][2:4])]
    print("Number of puzzles after filtering for different 1st and 9th targets:", len(puzzles)) #60624

    puzzles = puzzles[puzzles.principal_variation.apply(lambda x: x[2][2:4] != x[8][2:4])]
    print("Number of puzzles after filtering for different 3rd and 9th targets:", len(puzzles)) #58417

    puzzles = puzzles[puzzles.principal_variation.apply(lambda x: x[4][2:4] != x[8][2:4])]
    print("Number of puzzles after filtering for different 5th and 9th targets:", len(puzzles)) #55688

    puzzles = puzzles[puzzles.principal_variation.apply(lambda x: x[6][2:4] != x[8][2:4])]
    print("Number of puzzles after filtering for different 7th and 9th targets:", len(puzzles)) #49530
    ...
    print(f"Number of puzzles after filtering for hard and forcing second move: {len(puzzles)}") #18187
    ...
    print(f"Number of puzzles after filtering for correctness: {len(puzzles)}") #4707
    print("poop")

    with open(base_dir / "puzzles/interesting_puzzles_pv9_without_corruptions.pkl", "wb") as f:
