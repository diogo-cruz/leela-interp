...
def main(args):
    ...
    puzzles = puzzles[puzzles.principal_variation.apply(lambda x:len(x) == 5)]
    print("Number of puzzles after filtering for PV length 5:", len(puzzles)) #1013070

    # puzzles = puzzles[puzzles.principal_variation.apply(lambda x: x[0][2:4] != x[4][2:4])]
    # print("Number of puzzles after filtering for different 1st and 5th targets:", len(puzzles))

    # puzzles = puzzles[puzzles.principal_variation.apply(lambda x: x[2][2:4] != x[4][2:4])]
    # print("Number of puzzles after filtering for different 3rd and 5th targets:", len(puzzles))

    # puzzles = puzzles[puzzles.principal_variation.apply(lambda x: x[4][2:4] != x[6][2:4])]
    # print("Number of puzzles after filtering for different 5th and 7th targets:", len(puzzles))
    ...
    with open(base_dir / "puzzles/interesting_puzzles_pv5_without_corruptions.pkl", "wb") as f:
