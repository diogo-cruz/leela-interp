...
def main(args):
    ...
    puzzles["principal_variation"] = pd.Series(
        [p.Moves.split(" ")[1:1+puzzle_len] for p in puzzles.itertuples()], index=puzzles.index
    )
    puzzles = puzzles[puzzles.principal_variation.apply(lambda x:len(x) >= puzzle_len)]
    print(f"Number of puzzles after filtering for PV length >= {puzzle_len}:", len(puzzles))
    ...
    with open(base_dir / "puzzles/interesting_puzzles_alternate_without_corruptions.pkl", "wb") as f:
